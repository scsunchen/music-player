<template>
  <div class="rt-page">
    <!-- Noise overlay -->
    <div class="noise-overlay"></div>

    <!-- Magenta ambient light -->
    <div class="magenta-ambient"></div>

    <!-- Hero Section -->
    <section class="hero">
      <div class="hero-content">
        <span class="hero-era">2003 — 2010</span>
        <h1 class="hero-title">{{ data.hero.title }}</h1>
        <p class="hero-subtitle">{{ data.hero.subtitle }}</p>
        <p class="hero-intro">{{ data.hero.intro }}</p>
        <div class="hero-cta">
          <span class="cta-arrow">▼</span>
          <span class="cta-text">{{ data.hero.ctaText }}</span>
        </div>
      </div>
    </section>

    <!-- Timeline -->
    <section class="timeline">
      <div class="timeline-line"></div>

      <div
        v-for="(entry, index) in entriesWithSongs"
        :key="entry.songId"
        class="timeline-entry"
        :class="{
          'entry-left': index % 2 === 0,
          'entry-right': index % 2 === 1,
          'is-visible': visibleRows.has(index),
          'is-playing': currentPlayingId === entry.songId
        }"
        :data-index="index"
      >
        <!-- Dot with number -->
        <div class="timeline-dot">
          <span class="dot-num">{{ String(index + 1).padStart(2, '0') }}</span>
        </div>

        <!-- Content card -->
        <div class="entry-card" @click.stop="playSong(entry)">
          <div class="entry-cover" @click.stop="playSong(entry)">
            <img :src="entry.cover" :alt="entry.title" loading="lazy" />
            <div class="cover-play" v-if="currentPlayingId !== entry.songId">
              <svg viewBox="0 0 24 24" width="16" height="16"><path fill="#fff" d="M8 5v14l11-7z"/></svg>
            </div>
            <div class="cover-vinyl" v-if="currentPlayingId === entry.songId">
              <div class="vinyl-disc"></div>
            </div>
          </div>
          <h3 class="entry-song">{{ entry.title }}</h3>
          <span class="entry-artist">{{ entry.artist }}</span>
          <p class="entry-narration">{{ entry.narration }}</p>
        </div>
      </div>
    </section>

    <!-- Ending Section -->
    <section class="ending">
      <div class="ending-content">
        <div class="ending-line"></div>
        <p class="ending-text">{{ data.ending.text }}</p>
        <p class="ending-tagline">{{ data.ending.tagline }}</p>
        <button class="ending-cta" @click="router.push('/')">
          <span>{{ data.ending.ctaText }}</span>
          <svg viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6 1.41-1.41z"/></svg>
        </button>
      </div>
    </section>
  </div>
</template>

<script setup>
import { ref, reactive, computed, onMounted, onBeforeUnmount } from 'vue'
import { useRouter } from 'vue-router'
import { usePlayerStore } from '../stores/player'
import ringtonesData from '../config/ringtonesPlaylist'

const router = useRouter()
const playerStore = usePlayerStore()
const data = ringtonesData

const visibleRows = reactive(new Set())
const currentPlayingId = ref(-1)
let observer = null

const entriesWithSongs = computed(() => {
  return data.entries.map(entry => {
    const song = playerStore.songs.find(s => s.id === entry.songId)
    return {
      ...entry,
      cover: song ? song.cover : '/music-player/images/covers/1685cff3aa4f_cover.jpg',
      audioUrl: song ? song.audioUrl : ''
    }
  })
})

onMounted(() => {
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        const index = Number(entry.target.dataset.index)
        if (entry.isIntersecting) visibleRows.add(index)
      })
    },
    { threshold: 0.15, rootMargin: '0px 0px -40px 0px' }
  )
  setTimeout(() => {
    document.querySelectorAll('.timeline-entry').forEach(el => observer.observe(el))
  }, 100)
})

onBeforeUnmount(() => {
  if (observer) observer.disconnect()
})

const playSong = (entry) => {
  if (currentPlayingId.value === entry.songId) {
    playerStore.togglePlay()
    return
  }
  const song = playerStore.songs.find(s => s.id === entry.songId)
  if (song) {
    currentPlayingId.value = entry.songId
    playerStore.playSong(song)
  }
}
</script>

<style scoped>
:root {
  --rt-magenta: #ff2d78;
  --rt-cyan: #00e5ff;
  --rt-dark: #0a0812;
  --rt-card: rgba(255, 45, 120, 0.04);
  --rt-border: rgba(255, 45, 120, 0.1);
  --rt-text: rgba(255, 255, 255, 0.85);
  --rt-text-dim: rgba(255, 255, 255, 0.4);
}

.rt-page {
  min-height: 100vh;
  background: var(--rt-dark);
  overflow-x: hidden;
  position: relative;
  color: var(--rt-text);
  font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'PingFang SC',
    'Hiragino Sans GB', 'Microsoft YaHei', sans-serif;
  -webkit-font-smoothing: antialiased;
}

/* ===== Noise Overlay ===== */
.noise-overlay {
  position: fixed;
  inset: 0;
  pointer-events: none;
  z-index: 100;
  opacity: 0.03;
  background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noise)'/%3E%3C/svg%3E");
  background-size: 256px;
}

/* ===== Magenta Ambient ===== */
.magenta-ambient {
  position: fixed;
  top: -200px;
  left: 50%;
  transform: translateX(-50%);
  width: 600px;
  height: 600px;
  border-radius: 50%;
  background: radial-gradient(circle, rgba(255,45,120,0.1) 0%, transparent 70%);
  pointer-events: none;
  z-index: 0;
}

/* ===== Hero ===== */
.hero {
  min-height: 100vh;
  min-height: 100svh;
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  padding: 40px 32px 80px;
  position: relative;
  z-index: 1;
}
.hero-content { max-width: 720px; }
.hero-era {
  display: inline-block;
  font-size: 12px;
  letter-spacing: 8px;
  color: var(--rt-magenta);
  opacity: 0.7;
  margin-bottom: 24px;
}
.hero-title {
  font-size: clamp(48px, 10vw, 88px);
  font-weight: 900;
  color: var(--rt-magenta);
  font-family: 'Noto Serif SC', 'Source Han Serif SC', serif;
  letter-spacing: -2px;
  line-height: 1.1;
  margin: 0 0 16px;
  text-shadow: 0 0 80px rgba(255,45,120,0.3), 0 0 160px rgba(255,45,120,0.1);
}
.hero-subtitle {
  font-size: 14px;
  letter-spacing: 6px;
  color: var(--rt-text-dim);
  margin: 0 0 40px;
}
.hero-intro {
  max-width: 640px;
  margin: 0 auto 60px;
  font-size: 15px;
  line-height: 2;
  color: var(--rt-text-dim);
  text-align: justify;
}
.hero-cta {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 8px;
}
.cta-arrow {
  font-size: 14px;
  color: var(--rt-magenta);
  animation: bounceArrow 2s ease-in-out infinite;
  line-height: 1;
}
.cta-text {
  font-size: 12px;
  letter-spacing: 4px;
  color: var(--rt-magenta);
  opacity: 0.6;
}

/* ===== Timeline ===== */
.timeline {
  max-width: 900px;
  margin: 0 auto;
  padding: 60px 32px 100px;
  position: relative;
  z-index: 1;
}

.timeline-line {
  position: absolute;
  left: 24px;
  top: 0;
  bottom: 0;
  width: 1px;
  background: linear-gradient(to bottom, transparent, rgba(255,45,120,0.2), var(--rt-border), transparent);
}

/* ===== Timeline Entry ===== */
.timeline-entry {
  position: relative;
  margin-bottom: 32px;
  opacity: 0;
  transition: all 0.7s cubic-bezier(0.16, 1, 0.3, 1);
  transform: translateY(15px);
}

.timeline-entry:last-child { margin-bottom: 0; }

.timeline-entry.is-visible {
  opacity: 1;
  transform: translateY(0);
}

.entry-left.is-visible { transform: translateY(0); }
.entry-right.is-visible { transform: translateY(0); }

/* ===== Timeline Dot ===== */
.timeline-dot {
  position: absolute;
  left: 24px;
  top: 16px;
  transform: translateX(-50%);
  width: 10px;
  height: 10px;
  border-radius: 50%;
  border: 2px solid rgba(255,45,120,0.4);
  background: var(--rt-dark);
  z-index: 2;
  transition: all 0.4s;
}

.is-playing .timeline-dot {
  background: var(--rt-magenta);
  border-color: var(--rt-magenta);
  box-shadow: 0 0 12px rgba(255,45,120,0.4);
}

.dot-num {
  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
  font-size: 0;
  color: var(--rt-magenta);
  white-space: nowrap;
  pointer-events: none;
}

/* ===== Entry Card ===== */
.entry-card {
  margin-left: 56px;
  background: var(--rt-card);
  border: 1px solid var(--rt-border);
  border-radius: 14px;
  padding: 18px;
  backdrop-filter: blur(10px);
  -webkit-backdrop-filter: blur(10px);
  transition: all 0.4s;
  cursor: pointer;
}

.entry-card::after {
  content: '';
  display: table;
  clear: both;
}

.is-playing .entry-card {
  border-color: rgba(255,45,120,0.3);
  box-shadow: 0 0 30px rgba(255,45,120,0.08);
}

.entry-card:hover {
  border-color: rgba(255,45,120,0.2);
  background: rgba(255,45,120,0.06);
}

/* ===== Entry Cover (float for text wrap) ===== */
.entry-cover {
  position: relative;
  width: 80px;
  height: 80px;
  border-radius: 10px;
  overflow: hidden;
  flex-shrink: 0;
  cursor: pointer;
  background: rgba(255,45,120,0.06);
  float: right;
  margin-left: 14px;
  margin-bottom: 6px;
}

.entry-right .entry-cover {
  float: left;
  margin-left: 0;
  margin-right: 14px;
}

.entry-cover img {
  width: 100%;
  height: 100%;
  object-fit: cover;
  display: block;
}

.cover-play {
  position: absolute;
  inset: 0;
  display: flex;
  align-items: center;
  justify-content: center;
  background: rgba(0,0,0,0.4);
  opacity: 0;
  transition: opacity 0.3s;
}

.entry-cover:hover .cover-play { opacity: 1; }

.cover-vinyl {
  position: absolute;
  inset: 0;
  display: flex;
  align-items: center;
  justify-content: center;
  background: rgba(0,0,0,0.5);
}

.vinyl-disc {
  width: 40px;
  height: 40px;
  border-radius: 50%;
  background: conic-gradient(from 0deg, #1a1a1a, #333, #1a1a1a, #333, #1a1a1a);
  animation: vinylSpin 3s linear infinite;
  box-shadow: inset 0 0 8px rgba(0,0,0,0.5);
}

/* ===== Entry Info ===== */
.entry-song {
  margin: 0 0 2px;
  font-size: 17px;
  font-weight: 700;
  color: #fff;
  font-family: 'Noto Serif SC', 'Source Han Serif SC', serif;
  line-height: 1.3;
}

.entry-artist {
  font-size: 12px;
  color: var(--rt-magenta);
  opacity: 0.6;
  margin-bottom: 10px;
  display: block;
}

.entry-narration {
  font-size: 13px;
  line-height: 1.85;
  color: var(--rt-text-dim);
  margin: 0;
}

/* ===== Ending ===== */
.ending {
  padding: 100px 32px 120px;
  text-align: center;
  max-width: 640px;
  margin: 0 auto;
  position: relative;
  z-index: 1;
}
.ending-content { display: flex; flex-direction: column; gap: 0; }
.ending-line {
  width: 60px;
  height: 1px;
  background: var(--rt-magenta);
  opacity: 0.5;
  margin: 0 auto 48px;
}
.ending-text {
  font-size: 16px;
  line-height: 2.2;
  color: var(--rt-text);
  margin: 0 0 32px;
  font-style: italic;
}
.ending-tagline {
  font-size: 18px;
  font-weight: 700;
  color: var(--rt-magenta);
  font-family: 'Noto Serif SC', 'Source Han Serif SC', serif;
  margin: 0 0 48px;
}
.ending-cta {
  display: inline-flex;
  align-items: center;
  gap: 8px;
  padding: 12px 32px;
  border: 1px solid rgba(255,45,120,0.4);
  border-radius: 40px;
  background: transparent;
  color: var(--rt-magenta);
  font-size: 14px;
  cursor: pointer;
  transition: all 0.3s;
}
.ending-cta:hover {
  background: rgba(255,45,120,0.1);
  border-color: var(--rt-magenta);
}

/* ===== Responsive: Desktop ===== */
@media (min-width: 768px) {
  .timeline { padding: 60px 48px 100px; }

  .timeline-line {
    left: 50%;
    transform: translateX(-50%);
  }

  .timeline-dot {
    left: 50%;
  }

  .dot-num {
    top: -22px;
    left: 50%;
    transform: translateX(-50%);
    font-size: 11px;
    font-weight: 600;
    opacity: 0.5;
  }

  .entry-left .entry-card {
    margin-left: calc(50% + 36px);
  }

  .entry-right .entry-card {
    margin-left: 0;
    margin-right: calc(50% + 36px);
  }

  .entry-right .timeline-dot {
    left: 50%;
  }

  .entry-cover {
    width: 100px;
    height: 100px;
  }

  .entry-song {
    font-size: 19px;
  }
}

/* ===== Responsive: Large Desktop ===== */
@media (min-width: 1024px) {
  .timeline { max-width: 1100px; padding: 60px 64px 100px; }

  .entry-left .entry-card {
    margin-left: calc(50% + 48px);
  }

  .entry-right .entry-card {
    margin-right: calc(50% + 48px);
  }
}

/* ===== Animations ===== */
@keyframes bounceArrow {
  0%, 100% { transform: translateY(0); }
  50% { transform: translateY(8px); }
}

@keyframes vinylSpin {
  from { transform: rotate(0deg); }
  to { transform: rotate(360deg); }
}
</style>
